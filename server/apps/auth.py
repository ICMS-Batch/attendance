...
def login():
    ...
    if request.method == "POST":
        ...
        data_from_database = Student.query.filter_by(email=auth_email).first()
...
def admin_signup():
    if request.method == "POST":
        ...
        data_from_database = Admin.query.filter_by(email=email).first()
...
def admin_login():
    ...
    if request.method == "POST":
        ...
        # now fetching the data from the database
        data_from_database = Admin.query.filter_by(email=auth_email).first()
        if data_from_database != None:
            if data_from_database.password == auth_pass:
                return "Login Successful"
            else:
